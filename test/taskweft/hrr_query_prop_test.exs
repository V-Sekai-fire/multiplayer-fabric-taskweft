defmodule Taskweft.HRR.QueryPropTest do
  ...
  @dim 64
  @source "test_items"
  @table :hrr_query_prop_table

  setup_all do
    path =
      Path.join(System.tmp_dir!(), "hrr_query_test_#{:erlang.unique_integer([:positive])}.dets")
  ...
  property "WHERE field == non-existent value returns []" do
    forall records <- record_list_gen() do
      with_storage(fn store ->
        ...
        result =
          Query.execute(store, :all, query, ["__NO_MATCH__#{:erlang.unique_integer()}__"], [])
  ...
  property "WHERE field != some_value union WHERE field == some_value = all rows" do
    forall records <- record_list_gen() do
      with_storage(fn store ->
        populate(store, records)
  ...
  property "ORDER BY ASC and DESC are consistent" do
    forall records <- record_list_gen() do
      with_storage(fn store ->
        populate(store, records)
